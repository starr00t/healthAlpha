...
export default function NoteForm({ date, onClose, onSuccess, noteId }: NoteFormProps) {
  ...
  useEffect(() => {
    const changed = 
      ...
      JSON.stringify(photos) !== JSON.stringify(initialData.photos) ||
      JSON.stringify(videos) !== JSON.stringify(initialData.videos);
    
    setHasChanges(changed);
    if (changed && saveSuccess) {
      setSaveSuccess(false); // 변경되면 저장 완료 상태 해제
    }
  }, [title, content, photos, videos, initialData, saveSuccess]);
        ...
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 p-4">
          ...
          <div className="flex gap-2 md:gap-3">
            ...
            <button
              ...
              className="flex-1 px-4 py-3 md:py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors font-semibold min-h-[44px] md:min-h-0 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isSaving ? (
                <>
